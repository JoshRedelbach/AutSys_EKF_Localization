...
def display_map_with_points_map_frame(image, map_points, origin, resolution, estimated_points=None, covariances=None):
    """
    ...
    # Draw covariance ellipses around each n-th estimated point
    if covariances is not None and estimated_points is not None:
        for (pt, cov_vals) in zip(estimated_points, covariances):
            cov_matrix = np.array([[cov_vals[0], cov_vals[1]],
            ...
            # Width and height = 2*sqrt of eigenvalues (for 1-sigma ellipse)
    ...
    for i in range(num_points - 1):
        ...
        if i == 1:
            ...
            plt.plot([p1[0], p2[0]], [p1[1], p2[1]],
                     color=line_color, linestyle=line_style, linewidth=1.5, label="Kidnapping Path")
        elif i == 0:
            line_color = 'green'
            line_style = '-'
            arrow_color = 'green'
            plt.plot([p1[0], p2[0]], [p1[1], p2[1]],
                     color=line_color, linestyle=line_style, linewidth=1.5, label="True Trajectory")
        else:
            line_color = 'green'
            line_style = '-'
            arrow_color = 'green'
        ...
        plt.arrow(arrow_start[0], arrow_start[1], arrow_dx, arrow_dy,
                  head_width=0.15, head_length=0.2, fc=arrow_color, ec=arrow_color, length_includes_head=True)
...
# --- Kidnapping path ---
kidnapping_pt1 = rectangle_pt1
kidnapping_pt2 = rectangle_pt2
kidnapping_pt3 = complex1_pt1
kidnapping_pt4 = complex1_pt2
kidnapping_pt5 = complex1_pt3
kidnapping_pt6 = complex1_pt4
kidnapping_pt7 = complex1_pt5
kidnapping_pt8 = complex1_pt6
kidnapping_pt9 = complex1_pt7
kidnapping_pt10 = complex1_pt8        
kidnapping_points = np.array([kidnapping_pt1, kidnapping_pt2, kidnapping_pt3, kidnapping_pt4,
                             kidnapping_pt5, kidnapping_pt6, kidnapping_pt7, kidnapping_pt8,
                             kidnapping_pt9, kidnapping_pt10])
...
transformed_points = np.array([transform_point(pt, T_map_to_mapOG) for pt in kidnapping_points])

# --- Load estimated trajectory from CSV file ---
csv_path = "Final_Data/4_Kidnapping/ekf_trajectory_2.csv"

df = pd.read_csv(csv_path)

n = 5  # Change this to sample every n-th point
estimated_points = df[['x', 'y']].to_numpy()[::n]
covariances = df[['sigma_00', 'sigma_01', 'sigma_10', 'sigma_11']].to_numpy()[::n]
...
display_map_with_points_map_frame(image, transformed_points, [-10, -10], px_resolution, estimated_points, covariances=None)
