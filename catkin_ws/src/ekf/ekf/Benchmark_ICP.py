...
def icp_pipeline(src_points, dst_points, init_pose=None, ransac_iter=10, ransac_samples=4, ransac_threshold=0.3, percentage_inliers=0.6, max_icp_iter=20, max_points=180):
    """
    Main ICP pipeline without GO-ICP option that performs downsampling, and calls the chosen ICP algorithm
    Args:
        src_points: Nx2 array of source points
        dst_points: Mx2 array of destination points
        init_pose: initial pose as a 3x3 transformation matrix (optional)
        ransac_iter: number of RANSAC iterations
        ransac_samples: number of point pairs to sample per RANSAC iteration
        ransac_threshold: distance threshold to count inliers
        percentage_inliers: minimum percentage of inliers required to consider a transformation valid
    ...
    # --- CHOOSE ICP METHOD ---
    # !! Uncomment the desired ICP method below !!
    # 1 - ICP PURE
