...
def downsample_scan(scan_points, max_points=360):
...
def icp_ransac_nested(src, dst, max_iter=10, dst_tree=None, ransac_iter=10, ransac_samples=4, ransac_threshold=0.3, best_n_inlier=200):
    """
...
def icp_ransac_first(src, dst, max_iter=10, dst_tree=None, ransac_iter=10, ransac_samples=4, ransac_threshold=0.3, best_n_inlier=200):
    """
...
def grid_search_go_icp(src_points, dst_points, 
                       rot_range=(-np.pi, np.pi), rot_step=np.deg2rad(45),
    ...
    grid_points = np.array([pt1, pt2, pt3, pt4, pt5, pt6, pt7, pt8,pt9, pt10, pt11]) 
...
def robust_icp_pipeline(src_points, dst_points, init_pose=None, ransac_iter=10, ransac_samples=4, ransac_threshold=0.3, best_n_inlier=200, max_icp_iter=20, max_points=360):
    
    dst_tree = KDTree(dst_points)
    
    # ---- Downsample Source Points ----
    if len(src_points) > max_points:
        src_points = downsample_scan(src_points, max_points)
    ...
    # T_refined, error = icp_ransac_nested(best_src, dst_points, max_iter=10, dst_tree=dst_tree, ransac_iter=5, ransac_samples=ransac_samples, ransac_threshold=ransac_threshold, best_n_inlier=best_n_inlier)

    # T_refined, error = icp_ransac_first(best_src, dst_points, max_iter=20, dst_tree=dst_tree, ransac_iter=5, ransac_samples=ransac_samples, ransac_threshold=ransac_threshold, best_n_inlier=best_n_inlier)

    final_T = T_refined @ T_best

    return final_T, error